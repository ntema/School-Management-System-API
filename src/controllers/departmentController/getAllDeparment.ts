...
export const getAllDeparment = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const count = await Department.find(option).countDocuments();
    ...
    const departments = await Department.find(option)
      .limit(limit * 1)
      .skip(skip);
    return res
      .status(200)
      .json({ status: "success", data: { ...result, count, pages, departments } });
  } catch (error) {
    next(error);
  }
};
